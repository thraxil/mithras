...
class Node(models.Model):
    ...
    def get_absolute_url(self):
        return "/users/%s/%ss/%04d/%02d/%02d/%s/" % (self.user.username,self.type,self.created.year,self.created.month,self.created.day,self.slug)
...
def scaled_tags():
    ...
    tags = [(t,t.node_set.all().count()) for t in Tag.objects.all().order_by("name")]
...
class Image(models.Model):
    ...
    def scaled_image_url(self):
        ...
        p = "/".join(list(sid))
        return "http://static.thraxil.org/pomelo_images/scaled/%s/%d.%s" % (p,self.node.id,self.ext)
...
class Comment(models.Model):
    ...
    def has_replies(self):
        return self.node.comment_set.filter(reply_to=self.id,status='approved').count() > 0

    def replies(self):
        return self.node.comment_set.filter(reply_to=self.id,status='approved')
    ...
    def has_author_url(self):
        ...
        if self.author_url == "": return False
